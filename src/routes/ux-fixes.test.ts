...
async function seedData() {
  await env.DB.batch([
    ...
    // Chunks with tags that cross episodes
    ...
    env.DB.prepare("INSERT INTO tags (name, slug, usage_count) VALUES ('platform', 'platform', 3)"),
    env.DB.prepare("INSERT INTO tags (name, slug, usage_count) VALUES ('llms', 'llms', 1)"),
    // Chunk-tag associations (ecosystem spans both episodes)
    ...
    env.DB.prepare("INSERT INTO chunk_tags (chunk_id, tag_id) VALUES (3, 3)"),
    env.DB.prepare("INSERT INTO chunk_tags (chunk_id, tag_id) VALUES (4, 1)"),
...
describe("Fix 3: Most connected chunks on homepage", () => {
  ...
  it("most-connected shows chunks with the highest tag overlap", async () => {
    const res = await SELF.fetch("http://localhost/");
    const html = await res.text();
    // eco-growth and platform-evolve share the 'ecosystem' tag and span episodes
...
describe("Fix 5: Thread following across episodes", () => {
  ...
  it("thread shows chunks from OTHER episodes sharing tags", async () => {
    const res = await SELF.fetch("http://localhost/chunks/eco-growth");
    const html = await res.text();
    // eco-growth (ep 2024-04-08) shares 'ecosystem' tag with eco-collapse (ep 2024-03-25)
