...
async function seedData() {
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO concordance (word, total_count, doc_count) VALUES ('ecosystem', 3, 2)"),
    env.DB.prepare("INSERT INTO concordance (word, total_count, doc_count) VALUES ('platform', 2, 1)"),
...
describe("GET /api/concordance", () => {
  it("returns word frequencies as JSON", async () => {
    const res = await SELF.fetch("http://localhost/api/concordance");
  ...
  it("filters by date range", async () => {
    const res = await SELF.fetch(
      "http://localhost/api/concordance?from=2024-03-01&to=2024-04-01"
    );
    const data = (await res.json()) as any;
