...
async function seedWithTopics() {
  await env.DB.batch([
    ...
    // Topics (tags table)
    env.DB.prepare("INSERT INTO tags (name, slug, usage_count) VALUES ('llms', 'llms', 10)"),
    env.DB.prepare("INSERT INTO tags (name, slug, usage_count) VALUES ('agent', 'agent', 8)"),
    env.DB.prepare("INSERT INTO tags (name, slug, usage_count) VALUES ('vibe coding', 'vibe-coding', 5)"),
    ...
    env.DB.prepare("INSERT INTO episode_tags (episode_id, tag_id) VALUES (1, 3)"),
    // Link topics to chunks
    env.DB.prepare("INSERT INTO chunk_tags (chunk_id, tag_id) VALUES (1, 1)"),
    env.DB.prepare("INSERT INTO chunk_tags (chunk_id, tag_id) VALUES (1, 2)"),
    env.DB.prepare("INSERT INTO chunk_tags (chunk_id, tag_id) VALUES (2, 3)"),
  ]);
}
...
describe("Episode page topic marginalia", () => {
  it("shows topics as always-visible marginalia without details/summary", async () => {
    ...
    // Extract the tags-margin aside element (up to its closing tag)
  ...
  it("does not show marginalia section when episode has no topics", async () => {
    ...
    const html = await res.text();
    expect(res.status).toBe(200);
    expect(html).not.toContain("tags-margin");
  });
});
...
describe("Topic links in marginalia", () => {
  it("episode page topic links point to /tags/:slug", async () => {
    const res = await SELF.fetch("http://localhost/episodes/2025-01-06-topics");
    const html = await res.text();
    expect(html).toContain('href="/tags/llms"');
    expect(html).toContain('href="/tags/agent"');
    expect(html).toContain('href="/tags/vibe-coding"');
  });

  it("chunk page topic links point to /tags/:slug", async () => {
    const res = await SELF.fetch("http://localhost/chunks/llm-agents-2025-01-06");
    const html = await res.text();
    expect(html).toContain('href="/tags/llms"');
    expect(html).toContain('href="/tags/agent"');
