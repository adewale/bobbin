...
describe("pipeline tuning rules", () => {
  ...
  it("promotes phrases only after enough chunk and episode support", () => {
    expect(getCandidatePromotionReason(
      ...
      { chunkSupport: 3, episodeSupport: 1, existingUsageCount: 0, wordDistinctiveness: 0, llmSupportCount: 0 }
    )).toBe("insufficient_episode_support");

    expect(getCandidatePromotionReason(
      { kind: "phrase", normalizedCandidate: "vibe coding" },
      { chunkSupport: 4, episodeSupport: 2, existingUsageCount: 0, wordDistinctiveness: 0, llmSupportCount: 0 }
    )).toBeNull();
  });
  ...
  it("rejects weak singleton concepts from corpus priors", () => {
    expect(getCorpusPriorRejectionReason(
      ...
      { chunkSupport: 2, episodeSupport: 1, existingUsageCount: 0, wordDistinctiveness: 4, llmSupportCount: 0 }
    )).toBeTruthy();

    expect(getCorpusPriorRejectionReason(
      { kind: "concept", normalizedCandidate: "llms" },
      { chunkSupport: 50, episodeSupport: 12, existingUsageCount: 100, wordDistinctiveness: 100, llmSupportCount: 0 }
    )).toBeNull();
  });
      ...
      fc.property(
        ...
        (kind, normalizedCandidate, chunkSupport, episodeSupport) => {
          const reason = getCandidatePromotionReason(
            ...
            { chunkSupport, episodeSupport, existingUsageCount: 0, wordDistinctiveness: 100, llmSupportCount: 0 }
          );
